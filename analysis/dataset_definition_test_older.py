...
for disease in diseases:
    ...
    dataset.preceding_registration_index = preceding_registration(index_date).exists_for_patient()
    dataset.preceding_registration_incident = preceding_registration(getattr(dataset, f"{disease}_inc_date")).exists_for_patient()
    ...
    prev_denominators[f"{disease}_prev_denom"] = (
            (getattr(dataset, f"{disease}_prev") == True | False)
        )
    ...
    incidence_numerators[f"{disease}_inc_num"] = (
            ...
            & (dataset.date_of_death.is_after(getattr(dataset, f"{disease}_inc_date")) | patients.date_of_death.is_null())
            & (dataset.preceding_registration_incident)
        )
    ...
    incidence_denominators[f"{disease}_inc_denom"] = (
            ...
            & (dataset.preceding_registration_index)
        )
...
dataset.define_population(
    ...
    & ((dataset.sex == "male") | (patients.sex == "female"))
    & (dataset.date_of_death.is_after(index_date) | patients.date_of_death.is_null())
